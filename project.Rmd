...
```
## Project 1
Members: Angelita Krepel, Adil Iqbal, Dosbol Aliev, 
...
summary(lr.glm)
```
(We are hiding the results because it will make the final too many pages, run it in the base code to see the results). Based on the summary of the logistic regression model we can see that not all the variables are important in predicting, so we will try again but this time with the variables deemed important.
